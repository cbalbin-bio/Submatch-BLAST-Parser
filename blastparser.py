...
class BLASTParser:
    def __init__(self, query_path, database, acc_seq=False, pdb_seqsolv=False, pdb_seqnums=False, taxids=False, evalue=2000000, max_target_seqs=2000000):
    ...
    def __run_and_parse_BLAST__(self):
        ...
        if self.__taxids__:
            result = subprocess.run(["blastp","-task", "blastp-short", "-db", self.__database__, "-query", self.__query_seq__, "-negative_taxidlist", self.__taxid_fpath__, "-evalue", self.__evalue__, "-max_target_seqs", self.__max_target_seqs__,"-gapopen", "32767", "-gapextend", "32767", "-outfmt", outfmt], capture_output=True)
        else:
            result = subprocess.run(["blastp", "-task", "blastp-short", "-db", self.__database__, "-query", self.__query_seq__, "-evalue", self.__evalue__, "-max_target_seqs", self.__max_target_seqs__, "-gapopen", "32767", "-gapextend", "32767", "-outfmt", outfmt], capture_output=True)
        ...
        for line in result.stdout.decode("utf-8").split("\n")[:-1]:
            ...
            hit_data = HitData(line[0], int(line[1]), int(line[2]), int(line[3]), int(line[4]), line[5], line[6], line[7], line[8], line[9], line[10])
